...
def is_in_range(range_list, position):
    for point in range_list:
        ...
        is_x = pos_x <= x + 0.1 and pos_x >= x - 0.1
        is_z = pos_z <= z + 0.1 and pos_z >= z - 0.1
...
def clear_non_walkable(map, avg_floor_z, avg_movement_z):
    ...
    for chunk_key in chunks:
        ...
        for block in chunk_blocks:
            ...
            if block_world_y > avg_floor_z + 0.2 and block_world_y < avg_movement_z and key not in clear_list:
            ...
            for j in range(-2, 3):
                ...
                if chunk_key in map["chunkCache"]:
                    ...
                    for block in map["chunkCache"][chunk_key]["blocks"]:
                        if(block["position"]["x"] == block_x and block["position"]["y"] == block_y and block["position"]["z"] == block_z):
